...
const user: User = { id: 1, email: "me@example.com", role: "admin", created_at: "" };
...
describe("signing in", () => {
  it("sends the credentials and hands back the user", async () => {
    ...
    expect((stub as unknown as { login: ReturnType<typeof vi.fn> }).login).toHaveBeenCalledWith(
      "me@example.com",
  ...
  it("puts a field-tagged rejection on the input it blames", async () => {
    const stub = api({
      login: vi.fn(() => Promise.reject(new ApiError(400, "a valid email address is required", null, "email"))),
    });
  ...
  it("turns a rate limit into a wait the user can act on", async () => {
    const stub = api({
      login: vi.fn(() => Promise.reject(new ApiError(429, "too many failed attempts, try again later", 900))),
    });
    render(<AuthScreen mode="login" onSignedIn={vi.fn()} api={stub} />);
    ...
    expect((await screen.findByRole("alert")).textContent).toContain("15 minutes");
  });

  it("says the server is unreachable rather than blaming the password", async () => {
    const stub = api({ login: vi.fn(() => Promise.reject(new TypeError("Failed to fetch"))) });
    render(<AuthScreen mode="login" onSignedIn={vi.fn()} api={stub} />);
...
describe("first-run setup", () => {
  ...
  it("tells a second tab to reload when setup was already completed elsewhere", async () => {
    const stub = api({
      createFirstUser: vi.fn(() => Promise.reject(new ApiError(409, "setup has already been completed"))),
    });
