// warning-browser exercises the real checker, runner, store, API and stream.
package main
...
func run() error {
	...
	user, err := db.CreateUser(ctx, "warning@example.invalid", password, store.RoleAdmin)
	if err != nil {
		return err
	}
	session, err := db.CreateSession(ctx, user.ID, "warning fixture", "127.0.0.1")
	if err != nil {
		return err
	}
	...
	ids := []int64{}
	for i := range 8 {
		m, err := db.CreateMonitor(ctx, store.Monitor{Name: fmt.Sprintf("Service %d", i), Type: "http", Target: fmt.Sprintf("%s/%d", target.URL, i), Enabled: true, Retries: 2, IntervalS: 300, CaptureResponse: true})
		if err != nil {
			return err
		}
	...
	for commands.Scan() {
		...
		if c.Healthy < 0 || c.Healthy >= 8 {
			return fmt.Errorf("invalid monitor")
		}
		before := alerts.Load()
		healthy[c.Healthy].Store(true)
		m, err := db.GetMonitor(ctx, ids[c.Healthy])
		if err != nil {
			return err
		}
		if _, err := runner.CheckNow(ctx, m); err != nil {
