...
func scanMonitor(s scanner) (Monitor, error) {
	...
	err := s.Scan(
		...
		&headersJSON, &body, &m.SSLWarnDays, &m.Enabled,
...
func (db *DB) CreateMonitor(ctx context.Context, m Monitor) (Monitor, error) {
	...
	res, err := tx.ExecContext(ctx, `
		...
		headersJSON, nullString(m.Body), m.SSLWarnDays, m.Enabled,
...
func (db *DB) updateMonitor(ctx context.Context, m Monitor, expected []int64) (Monitor, error) {
	...
	args := []any{
		...
		m.Enabled, nullInt(m.PushIntervalS), pushGraceValue(m),
		next, m.ID,
	}
...
const updateMonitorSetClause = `
	UPDATE monitors SET
		...
		enabled = ?, push_interval_s = ?, push_grace_s = ?,
