...
const heartbeatColumns = `id, monitor_id, ts, ok, latency_ms, status_code, error, assessment, failure_kind, maintenance`
...
func scanHeartbeat(s scanner) (Heartbeat, error) {
	...
	if err := s.Scan(&hb.ID, &hb.MonitorID, &ts, &hb.OK, &latency, &status, &errText, &hb.Assessment, &hb.FailureKind, &hb.Maintenance); err != nil {
...
func (db *DB) Uptime(ctx context.Context, monitorID int64, window time.Duration) (UptimeStats, error) {
	...
	var (
		stats                                 UptimeStats
		rawTotal                              int
		rawWarning, rawLegacy, rawMaintenance sql.NullInt64
		rawUp                                 sql.NullInt64
		rawLatSum                             sql.NullFloat64
		rawLatN                               int
	)
	err := db.Reader.QueryRowContext(ctx, `
		SELECT count(CASE WHEN maintenance = 0 AND assessment IN ('up','down') THEN 1 END), sum(maintenance = 0 AND assessment = 'up'), sum(latency_ms), count(latency_ms), sum(maintenance = 0 AND assessment = 'warning'), sum(maintenance = 0 AND assessment = ''), sum(maintenance)
		FROM heartbeats
		WHERE monitor_id = ? AND ts >= ?`, monitorID, since,
	).Scan(&rawTotal, &rawUp, &rawLatSum, &rawLatN, &rawWarning, &rawLegacy, &rawMaintenance)
	if err != nil {
	...
	var (
		aggUp                                 sql.NullInt64
		aggWarning, aggLegacy, aggMaintenance sql.NullInt64
	...
	err = db.Reader.QueryRowContext(ctx, `
		SELECT sum(assessed_up), sum(assessed_down),
		       sum(COALESCE(latency_avg, 0) * latency_count), sum(latency_count), sum(warning_count), sum(up_count + down_count - assessed_up - assessed_down - warning_count - maintenance_count), sum(maintenance_count)
		FROM heartbeat_hourly
		WHERE monitor_id = ? AND bucket >= ?`, monitorID, since,
	).Scan(&aggUp, &aggDown, &aggLatSum, &aggLatN, &aggWarning, &aggLegacy, &aggMaintenance)
	if err != nil {
		return UptimeStats{}, fmt.Errorf("compute uptime for monitor %d: %w", monitorID, err)
	}

	stats.Maintenance = int(rawMaintenance.Int64 + aggMaintenance.Int64)
