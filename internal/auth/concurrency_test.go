...
// TestVerifyPasswordBoundsConcurrency is the memory ceiling, stated as a test.
//
...
func TestVerifyPasswordBoundsConcurrency(t *testing.T) {
	...
	if got := peak.Load(); got > maxConcurrentVerifies {
		t.Errorf("peak concurrent verifications = %d, want at most %d — "+
			"each one holds %d MiB", got, maxConcurrentVerifies, argonMemory/1024)
	}
}
